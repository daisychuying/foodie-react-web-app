            ...
            {currentUser && currentUser.role !== "CHEF" &&
                <>
                    <div className="mt-3 mb-5">
                        <textarea className="form-control" onChange={(e) => setQuestion(e.target.value)} value={question} placeholder="Write down your question, our chef will be willing to answer!"/>
                ...
                {questions && questions.map((question, index) =>
                    <div key={index} className="list-group-item">
                        ...
                        {(currentUser && !question.answer && question.post.author._id === currentUser._id && !isReplying) &&
                            <button onClick={() => setIsReplying(true)} className="btn btn-warning btn-sm">Reply</button>
                        }
                        {question.answer &&
                            <>
                                ...
                                {(currentUser && question.post.author._id === currentUser._id && !isReplying) &&
                                    <button onClick={() => setIsReplying(true)} className="btn btn-warning btn-sm">Edit</button>
                                }
                            </>
                        }
                        {isReplying &&
                            <>
                                ...
                                <button onClick={() => setIsReplying(false)} className="btn btn-sm btn-info float-end ms-2">Cancel</button>
